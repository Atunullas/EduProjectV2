...
<div class="container section">
	<form action="nextQuest.do" method="post" class="form-horizontal">
		...
		<div class="row">
			<div class="form-group text-center">Question no : ${curQuestNo+1}
				out of ${noOfTotalQuizQues}</div>
		</div>
		...
		<div class="row">
			...
			<c:forEach items="${curQuestion.options}" var="eachOption">
				<div>
					<input type="checkbox" name="selOpt" value="${eachOption.optionId}" id="${eachOption.optionId}" />&nbsp;
					<label>${eachOption.optionTxt}</label>
				</div>
			</c:forEach>
		</div>
		<input type="hidden" name="curQuestionID" value="${curQuestNo}">
		<hr>
		<div class="row">
			<div class="form-group">
